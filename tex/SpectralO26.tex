...
\begin{document}
  ...
  \begin{abstract}
    ...
    representation-theoretic identification (Level~III), conjectured to take place
    in an isotypic sector of the binary icosahedral group $2I$ along the admissibility
    thread $Q_8 \subset 2I \subset \mathrm{SU}(2)$.
  ...
  \section{Background and motivation}
    ...
    scaling laws from the bounded-flux dynamics of the Cosmochrony substrate~$\chi$~\cite{Cosmochrony}.
...
$\delta_{\mathrm{pair}}$ stability holds.
  ...
  \subsection{Level III: Canonical identification (conjectural)}
    \label{sec:level3}

    The strong form requires the proportionality constant to be fixed canonically by
    the representation theory of $2I$, not merely absorbed into a normalisation convention.

    \begin{conjecture}[Level~III]
      \label{conj:level3}
      There exists an irreducible representation $\rho$ of $2I$, selected by the
      admissibility thread $Q_8 \subset 2I \subset \mathrm{SU}(2)$, such that for all
      admissible pairs $\{c, q-c\}$ and all $n$ in the pre-saturation regime:
      \begin{enumerate}[label=(\roman*)]
    \item the residual vector $v_c^{(n)}$ lies in $V_\rho$ under a canonical
    embedding determined by $\rho$ and $q$;
    \item the un-normalised matrix coefficient
    $\widetilde{M}_n = v_c^{(n)} \otimes \overline{v_{q-c}^{(n)}}$
    takes values in $\mathrm{End}(V_\rho)$;
    \item the proportionality constant between $\sigma^{\mathrm{can}}_{\mathrm{pair}}(n)$
    and $\|\widetilde{M}_n\|_{\mathrm{HS}}^2$ equals $d_\rho^{-2}$,
    the inverse squared dimension of $\rho$, up to the universal
    BFS-geometric factor $|S_n|^2$.
      \end{enumerate}
    \end{conjecture}
  ...
  \subsection{Candidate sector}
    ...
    Conjecture~\ref{conj:level3}(iii) would then read: the proportionality constant
    is $d_\rho^{-2} = \tfrac{1}{4}$, and $\widetilde{M}_n$ takes values in a
    $4$-dimensional space $\mathrm{End}(V_\rho) \simeq \mathbb{C}^{2 \times 2}$.

    This prediction is falsifiable: a direct computation of $\mathrm{rank}(\mathcal{C}_c)$
    from the O25 pipeline data (see §\ref{sec:test4}) determines whether the
    trajectory of $\widetilde{M}_n$ is consistent with a $4$-dimensional ambient
    space or requires a larger sector.
  ...
  \subsection{Structural interpretation}
    ...
    At Level~III, $\sigma_{\mathrm{pair}}$ \emph{is} the restriction of a canonical
    Hermitian quadratic form on a representation space selected by the admissibility
    thread: the pair observable is no longer a derived bilinear product, but a norm.
  ...
  \begin{equation}
    ...
    \bigr)
  \end{equation}
  over the window $[n_0, n_1]$ used in O25 (Table~1 therein; DOI pending deposit).
  ...
  \subsection{Decision table}
    ...
    \begin{center}
      \begin{tabular}{l|l}
        ...
        Tests~1--5 pass & Level~III; Conjecture~\ref{conj:level3} confirmed \\
        \hline
      \end{tabular}
    \end{center}
  ...
  \subsection{From bilinear observable to canonical norm}
    ...
    At Level~III, $\sigma_{\mathrm{pair}}$ is the restriction of a canonical Hermitian
    quadratic form on $\mathrm{End}(V_\rho)$ selected by the admissibility thread
    $Q_8 \subset 2I \subset \mathrm{SU}(2)$.
    The emergence of the pair observable is interpreted as a \emph{quadratic
  completion}: the passage from a linear object $v_c^{(n)} \in V_\rho$ to a
    rank-one operator $\widetilde{M}_n \in \mathrm{End}(V_\rho)$ endowed with its
    natural norm.
  ...
  \subsection{Outlook}
    ...
    $\delta_{\mathrm{pair}}$.
    The tests of §\ref{sec:tests} can be executed on O25 data without additional
    computation.
