...
$\delta_{\mathrm{pair}}$ stability holds.
  ...
  \subsection{Level III: Conditional theorem}
    ...
    \begin{remark}
      ...
      hypothesis, without
      identifying that carrier; the spin-$\tfrac{1}{2}$ sector itself is selected by minimality, not by
      Test~4.
    \end{remark}
  ...
  rather than $\mathrm{End}(V_\rho)$, the tested samples do not reach $d_\rho^2 = 4$, and the modal value
  ...
  $H_{\mathrm{eff}}$, not an identification of it.
    ...
    \begin{remark}[What Test~4 measures --- O29]
      ...
      That algebraic restriction alone does not exclude rank $4$; the non-attainment of
      $d_\rho^2 = 4$ is a result of the tested finite samples, and breaking the per-vector locking
      recovers four operator directions only in an auxiliary two-dimensional PCA truncation, not in
      $\mathrm{End}(V_\rho)$.
      At the checkpoints examined in O29, $r_{\mathrm{eff}} = 3$ is the modal numerical value in
      $\mathrm{End}(H_{\mathrm{eff}})$, not an invariant; the symmetric squares show a numerical $6 \to 3$
  ...
  \subsection{Decision table}
    ...
    \begin{center}
      \begin{tabular}{p{0.52\textwidth}|p{0.40\textwidth}}
        ...
        Tests~1--3 pass, $r_{\mathrm{eff}}$ fixed $\neq 3$ & inconsistent with the supplied three-dimensional adjoint
        reading; revise the candidate \\
        Tests~1--3 pass, $r_{\mathrm{eff}} = 3$ (compatible with the supplied adjoint spin-$1$ carrier), Test~5 fails
        & Level~II; carrier not identified, spin-$\tfrac{1}{2}$ target not decided by Test~4 \\
        Tests~1--5 pass & Finite-data evidence compatible with the supplied adjoint spin-$1$ carrier (O27--O29); no
    ...
    \begin{remark}
      The Level~III row records finite-data compatibility with a supplied carrier, not a carrier
      identification.
      The tested conjugate-pair samples do not reach $d_\rho^2 = 4$, and the locking-broken control
      recovers four operator directions only in an auxiliary PCA truncation.
      The value $r_{\mathrm{eff}} = 3$ was reported by O28~\cite{Beau2026a32} across the pairs of its
      perimeter; in the extended recomputation of O29~\cite{Beau2026a33} it is modal, not invariant, and
      compatible with the adjoint (spin-$1$) carrier $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$ conditional on O23's
      supplied carrier and O27's admissible-saturation hypothesis, which the data do not identify.
    \end{remark}

    The row ``Tests~1--3 pass, $r_{\mathrm{eff}}$ fixed $\neq 3$'' is
    structurally informative even as a negative result: it would be inconsistent with the supplied
    three-dimensional adjoint reading, pointing to a specific revision of the admissibility hierarchy argument.
  ...
  \subsection{Selection of the admissible sector}
    ...
    admissible spectral dynamics; a direct test of $V_\rho$ requires an independent observable, built from
    independently sampled vectors as characterised in O29, and the dictionary of §\ref{sec:dictionary}
  ...
  \subsection{Outlook}
    ...
    No carrier is identified numerically from O25 data: O29~\cite{Beau2026a33} finds a numerical
    $6 \to 3$ Veronese collapse and finite-precision constraint diagnostics compatible with the supplied
    ...
    \begin{remark}[Test~4 in O29]
      ...
      The tested conjugate-pair samples do not reach $d_\rho^2 = 4$, and the numerical $6 \to 3$
      Veronese collapse and $\mathfrak{so}(3)$ constraint pattern are compatible with the supplied adjoint
