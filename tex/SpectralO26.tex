...
$\delta_{\mathrm{pair}}$ stability holds.
  ...
  \subsection{Candidate sector}
    ...
    \begin{proposition}[Symmetric-square bound under involution equivariance]
      ...
      The covariance~\eqref{eq:covariance} built from these products has rank at most
      $d(d+1)/2$.
    ...
    \begin{proof}
      ...
      The range of $\mathcal{C}_c = N^{-1}\sum_n \mathrm{vec}(\widetilde{M}_n)\,
      \mathrm{vec}(\widetilde{M}_n)^\dagger$ is contained in the span of the vectors
      $\mathrm{vec}(\widetilde{M}_n)$, hence in $\mathrm{vec}(\mathrm{Sym}^2(V_\rho))$.
  ...
  They require no modification of the O25 pipeline and can be evaluated from the stored
  Gram--Schmidt data.
  ...
  (Remark~\ref{rem:criterion-reform}), and under condition~(a) of Hypothesis~\ref{hyp:embedding} its
  Level~III target is bounded by Proposition~\ref{prop:sym-bound}.
  ...
  \end{itemize}
  Under condition~(a) of Hypothesis~\ref{hyp:embedding}, neither of the first two outcomes can
  occur: Proposition~\ref{prop:sym-bound} bounds $r_{\mathrm{eff}}$ by $d_\rho(d_\rho+1)/2 < d_\rho^2$.
  ...
  $H_{\mathrm{eff}}$, not an identification of it.
    ...
    \begin{remark}[What Test~4 measures --- O29]
      ...
      $\mathrm{Sym}(H_{\mathrm{eff}}, \mathbb{C})$; the non-attainment of
  ...
  \subsection{Decision table}
    ...
    \begin{center}
      \begin{tabular}{p{0.52\textwidth}|p{0.40\textwidth}}
        ...
        Tests~1--3 pass, $r_{\mathrm{eff}}$ fixed $\neq 3$ & not the modal value observed in
        $\mathrm{End}(H_{\mathrm{eff}})$; a bare
        effective rank does not by itself bear on the carrier (O29) \\
        ...
        Tests~1--5 pass & Finite-data evidence compatible with the supplied adjoint spin-$1$ carrier (rank data of
        O28--O29); no
    ...
    \begin{remark}
      The last row records finite-data compatibility with a supplied carrier, not a carrier
      identification, and no row can support Level~III through $r_{\mathrm{eff}} = d_\rho^2$ under
      condition~(a) of Hypothesis~\ref{hyp:embedding} (Proposition~\ref{prop:sym-bound}).
      Test~1 holds by construction and does not discriminate between rows.
      ...
      Because $r_{\mathrm{eff}}$ varies across pairs at $q = 101$ and $q = 211$, those data fall, for
      Test~4, in the row ``$r_{\mathrm{eff}}$ variable''; since they are formed in
      $\mathrm{End}(H_{\mathrm{eff}})$ rather than $\mathrm{End}(V_\rho)$, they decide no row of the table
      for Level~III.
    \end{remark}

    The row ``Tests~1--3 pass, $r_{\mathrm{eff}}$ fixed $\neq 3$'' is
    not decisive: a bare effective rank does not fix a representation dimension, and
    O29~\cite{Beau2026a33} records ranks four to nine at $q = 101$ and five at $q = 211$ on the same carrier,
    with modal rank three in multi-sample follow-ups.
  ...
  \subsection{Selection of the admissible sector}
    ...
    admissible spectral dynamics; a direct test of $V_\rho$ requires an independent observable and a typed bridge to
