...
$\delta_{\mathrm{pair}}$ stability holds.
  ...
  \subsection{Level III: Conditional theorem}
    ...
    \begin{remark}
      ...
      Test~4 of §\ref{sec:tests} identifies the adjoint carrier $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$
      underlying this construction (O29~\cite{Beau2026a33}); the spin-$\tfrac{1}{2}$ sector itself is selected
      by minimality, not by Test~4.
    \end{remark}
  ...
  following O29~\cite{Beau2026a33}: the anti-linear Born--Infeld parity makes $d_\rho^2$
  inaccessible from conjugate-pair data, and $r_{\mathrm{eff}} = 3$ identifies the adjoint
  (spin-$1$) carrier $H_{\mathrm{eff}}$ rather than the spin-$\tfrac{1}{2}$ target.
    \end{criterion}

    \begin{remark}[Carrier identification by Test~4 --- O29]
      \label{rem:criterion-reform}
      The Born--Infeld parity constraint $\rho_{q-c} = \bar\rho_c$ (O18) is anti-linear, so every outer
      product $\widetilde{M}_n$ is complex symmetric; the conjugate-pair target $d_\rho^2 = 4$ is therefore
      inaccessible by construction, and is restored by the locking-broken control of O29~\cite{Beau2026a33}.
      O29 identifies what Test~4 measures: $r_{\mathrm{eff}} = 3$ is the hard $6 \to 3$ Veronese-collapse
      invariant of the admissible trajectory, whose three constraint forms span $\mathfrak{so}(3)$ and whose
      action on $H_{\mathrm{eff}} = \mathbb{C}^3$ is irreducible.
      Test~4 thus identifies the irreducible adjoint carrier $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$
      (spin-$1$, vector) of O27~\cite{Beau2026a31}.
      It does not identify the spin-$\tfrac{1}{2}$ sector, which is the Schur target $V_\rho$ of
      $\bar\Phi : \mathfrak{su}(2) \to V_\rho$ and requires an independent observable; the spin-$\tfrac{1}{2}$
  ...
  \subsection{Decision table}
    ...
    \begin{center}
      \begin{tabular}{p{0.52\textwidth}|p{0.40\textwidth}}
        ...
        Tests~1--3 pass, $r_{\mathrm{eff}}$ fixed $\neq 3$ & carrier is not the three-dimensional adjoint; revise the candidate \\
        Tests~1--3 pass, $r_{\mathrm{eff}} = 3$ (irreducible adjoint spin-$1$ carrier), Test~5 fails & Level~II; carrier identified, spin-$\tfrac{1}{2}$ target not decided by Test~4 \\
        Tests~1--5 pass & Carrier identification complete (O27--O29): adjoint spin-$1$ \\
        \hline
      \end{tabular}
    \end{center}

    \begin{remark}
      The Level~III row records carrier identification.
      The anti-linear Born--Infeld parity makes the conjugate-pair target $d_\rho^2 = 4$ inaccessible
      (restored only by the locking-broken control), and $r_{\mathrm{eff}} = 3$ is the invariant of the
      irreducible adjoint (spin-$1$) carrier $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$, established in
      O28~\cite{Beau2026a32} and identified in O29~\cite{Beau2026a33}.
    \end{remark}

    The row ``Tests~1--3 pass, $r_{\mathrm{eff}}$ fixed $\neq 3$'' is
    structurally informative even as a negative result: it shows the carrier is not the three-dimensional
    adjoint, pointing to a specific revision of the admissibility hierarchy argument.
  ...
  \subsection{Interpretation of the exponent $\beta^*$}
    ...
    The transfer chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$ was
    established in O24~\cite{Beau2026a28}, unconditionally with respect to the fibre
    structure of~$\Pi$ (its standing conditions being the O4--O6 growth-equation closure
    and coupling hypotheses, the Weil realisation, and the power-law regime), where
    $\beta^*$ is defined via
    \begin{equation}
    ...
    non-abelian sector.
    This interpretation is made unconditional with respect to the sector identification
    in O27~\cite{Beau2026a31} (Corollary~6.1), where the rigidity theorem establishes
    that this non-abelian sector is necessarily $\mathfrak{su}(2)$ and not any other
    structure.
  ...
  \subsection{Selection of the admissible sector}
    ...
    Test~4 of §\ref{sec:test4} identifies the adjoint carrier $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$
    (spin-$1$, vector) on which the construction is based; the spin-$\tfrac{1}{2}$ selection rests on
    minimality and is not decided by Test~4 (O29~\cite{Beau2026a33}).
    The target space $\mathrm{End}(V_\rho) \simeq \mathbb{C}^{2 \times 2}$ is the minimal ambient space for
    admissible spectral dynamics; a direct test of $V_\rho$ requires the independent observable identified in
    O29, and the dictionary of §\ref{sec:dictionary} remains valid throughout.
  ...
  \subsection{Outlook}
    ...
    and O29~\cite{Beau2026a33} identifies it as the invariant of the irreducible adjoint (spin-$1$) carrier
    ...
    \]
    The numerical identification of the irreducible carrier from O25 data is completed in
    O29~\cite{Beau2026a33}: $r_{\mathrm{eff}} = 3$ is the Veronese-collapse invariant of the adjoint
    (spin-$1$) carrier $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$, closing the carrier-identification chain of
    O27~\cite{Beau2026a31}.
    The spin-$\tfrac{1}{2}$ sector is the Schur target $V_\rho$ of $\bar\Phi : \mathfrak{su}(2) \to V_\rho$,
    not identified by Test~4.

    \begin{remark}[Resolution of Test~4 in O29]
      \label{rem:test4-numerical}
      Test~4 is resolved in O29~\cite{Beau2026a33}.
      The anti-linear Born--Infeld parity makes the conjugate-pair target $d_\rho^2 = 4$ inaccessible
      (restored by the locking-broken control), and $r_{\mathrm{eff}} = 3$ is the hard $6 \to 3$
      Veronese-collapse invariant whose constraint forms span $\mathfrak{so}(3)$, identifying the irreducible
      adjoint (spin-$1$) carrier $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$ (see
      Remark~\ref{rem:criterion-reform}).
      Numerically, $r_{\mathrm{eff}} = 3$ holds universally for $q \in \{61, 151\}$
      (100\% of pairs, zero inter-pair variance) and modally for $q = 101$ by a multi-sample analysis.
      The spin-$\tfrac{1}{2}$ sector is the Schur target $V_\rho$ and is not decided by Test~4.
    \end{remark}
