...
$\delta_{\mathrm{pair}}$ stability holds.
  ...
  \subsection{Outlook}
    ...
    \]
    The remaining open question is the numerical identification of the correct
    irreducible sector from O25 data, i.e.\ the value of $d_\rho$ realised by Test~4.

    \begin{remark}[Status of Test~4: numerical attempt]
      \label{rem:test4-numerical}
      Test~4 has been partially implemented numerically on O25 data at
      $q \in \{29, 61\}$.
      The canonical construction $\Phi_{q,\rho} = \rho \circ \iota \circ \pi$
      was approximated by taking $\pi$ to be the projection onto the first three
      Gram--Schmidt basis vectors accumulated during the BFS cascade.
      This yields $r_{\mathrm{eff}} = 2$ universally across all pairs at $q = 61$,
      with $\sigma_1/\sigma_2 = 3.000$ exactly, and $r_{\mathrm{eff}} \in \{0, 2\}$
      at $q = 29$.
      The result is a structural artefact: the first Gram--Schmidt vector is the
      DC mode $e_0 = q^{-1/2}[1,\ldots,1]$, shared across all characters~$c$,
      so $\iota(\pi(v_s))$ collapses to a single direction in $\mathfrak{su}(2)$
      and $\mathcal{C}_c$ has rank at most~$2$.
      The correct implementation of $\pi$ requires identifying $H_{\mathrm{eff}}$
      with the isotypic subspace of $2I$ inside $\ell^2(G_q)$ and restricting
      the corresponding ADE eigenvectors of $\mathrm{Cay}(G_q, S_q)$ to the
      Weil representation space $\mathbb{C}^q$ via the Peter--Weyl coefficient map.
      This is an open problem in the spectral theory of Hecke operators on
      $\mathrm{Heis}_3(\mathbb{Z}/q\mathbb{Z})$, identified here as a direction
      for future work.
    \end{remark}
