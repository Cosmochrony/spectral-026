...
$\delta_{\mathrm{pair}}$ stability holds.
  ...
  \subsection{Candidate sector}
    ...
    \begin{proposition}[Symmetric-square bound under involution equivariance]
      ...
      formed as in~\eqref{eq:covariance}, has rank at most $d(d+1)/2$.
  ...
  outcomes can occur: Proposition~\ref{prop:sym-bound} bounds $r_{\mathrm{eff}}$ by
  ...
  \subsection{Decision table}
    ...
    \begin{center}
      \begin{tabular}{p{0.52\textwidth}|p{0.40\textwidth}}
        ...
        Tests~1--3 pass, $r_{\mathrm{eff}}$ variable & Level~III fails; at most Level~II \\
        ...
        Tests~1--3 pass, $r_{\mathrm{eff}} = 3$ (compatible with the supplied adjoint spin-$1$ carrier, and equal to
        the Level~III bound under condition~(a), hence not discriminating), Test~5 fails
    ...
    \begin{remark}
      ...
      The level labels of the table presuppose that Tests~1--3 have been run and that the covariance is
      formed from the embedded products in $\mathrm{End}(V_\rho)$.
      The O28 and O29 data meet neither condition: they are formed in $\mathrm{End}(H_{\mathrm{eff}})$ and
      include no run of Tests~1--3, so they occupy no row of the table.
      In single-sample runs their rank three is modal, with exceptions at $q = 101$ and $q = 211$; the
      multi-sample follow-ups of O29 give modal rank three for every exceptional pair.
    \end{remark}
